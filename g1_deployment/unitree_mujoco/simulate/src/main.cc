        ...
        if (m)
        {
          ...
          if (sim.run)
          {
                ...
                misaligned || sim.speed_changed)
            {
              ...
              // track robot pose
              sim.cam.lookat[0] = d->qpos[0];
              sim.cam.lookat[1] = d->qpos[1];
              sim.cam.lookat[2] = d->qpos[2];
              sim.cam.distance = 3.0;
              sim.cam.elevation = -5.0;
...
void *UnitreeSdk2BridgeThread(void *arg)
{
  ...
  // Set up pointer references so bridge can track model reloads
  interface->setPointerReferences(&m, &d);
