...
REGISTER_OBSERVATION(velocity_commands)
{
    ...
    // joystick->lx, ly, rx are in range [-1, 1]
    // so, we scale them to desired velocity ranges
    obs[0] = std::clamp(cfg["lin_vel_x"][1].as<float>() *joystick->ly(), cfg["lin_vel_x"][0].as<float>(), cfg["lin_vel_x"][1].as<float>());
    obs[1] = std::clamp(-cfg["lin_vel_y"][1].as<float>() * joystick->lx(), cfg["lin_vel_y"][0].as<float>(), cfg["lin_vel_y"][1].as<float>());
    obs[2] = std::clamp(-cfg["ang_vel_z"][1].as<float>() * joystick->rx(), cfg["ang_vel_z"][0].as<float>(), cfg["ang_vel_z"][1].as<float>());
